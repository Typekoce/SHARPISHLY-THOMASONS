...
class IngestionController:
    ...
    def consume():
        ...
        """
        # Ensure base path aligns with storage/snapshots
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../storage/snapshots'))
        IngestionController._ensure_dirs(base_dir)
    ...
    def update_php(job_id: int, status: str, chunks: list = None):
        ...
        """
        url = f"http://sharpishly.dev/php/ingestion/update/{job_id}"
        payload = {
            "status": status,
            "chunks": chunks or []
        }
